defmodule Clr.Air.Type do
  ...
  defp type(rest, [type], context, _loc, _bytes), do: {rest, [type], context}

  defp type(rest, [type, :comptime], context, _loc, _bytes) do
  ...
  defp general_types(rest, [{:ptr, kind, type, opts}, "?"], context, _loc, _bytes) do
    {rest, [{:ptr, kind, type, Keyword.put(opts, :optional, true)}], context}
  end

  defp general_types(rest, [type, "?"], context, _loc, _bytes) do
  ...
  defp array_type(rest, [type, "]", sentinel, ":", int, "["], context, _loc, _bytes) do
    {rest, [{:array, int, type, [sentinel: sentinel]}], context}
  end

  defp array_type(rest, [type, "]", int, "["], context, _loc, _bytes) do
    {rest, [{:array, int, type, []}], context}
  end

  defp fn_type(rest, [return_type | args_rest], context, _loc, _bytes) do
  ...
  defp sentinel(rest, ["null"], context, _loc, _bytes) do
    {rest, [:null], context}
  end

  defp sentinel(rest, ["0"], context, _loc, _bytes) do
    {rest, [0], context}
  end

  defp struct_type(rest, args, context, _loc, _bytes) do
    case Enum.reverse(args) do
      ["struct" | types] -> {rest, [{:struct, types}], context}
    end
  end

  defp sub_addr(rest, [c, b, a], context, _loc, _bytes), do: {rest, [{a, b, c}], context}

  defp sub_addr(rest, [d, c, b, a], context, _loc, _bytes), do: {rest, [{a, b, c, d}], context}

  # function post-traversals

  defp callconv(rest, [type, "callconv"], context, _loc, _bytes) do
  ...
  defp errorlist(rest, errors, context, _loc, _bytes) do
    {rest, [errors], context}
  end
end
